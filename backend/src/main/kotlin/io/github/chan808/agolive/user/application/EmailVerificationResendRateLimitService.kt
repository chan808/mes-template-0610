...
@Service
class EmailVerificationResendRateLimitService(private val rateLimiter: RateLimiter) {
    ...
    fun check(ip: String, email: String) {
        ...
        rateLimiter.retryAfterIfExceeded("RATE:EMAIL_VERIFY_RESEND:EMAIL:$email", ttlSeconds = 900, limit = 3)
            ?.also { log.warn("[RATE_LIMIT] email verification resend email limit exceeded email={} retryAfter={}s", maskEmail(email), it) }
            ?.let { throw RateLimitException(it) }
    }
}
