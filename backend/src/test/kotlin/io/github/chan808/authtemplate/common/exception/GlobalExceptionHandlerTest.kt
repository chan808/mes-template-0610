...
class GlobalExceptionHandlerTest {
    ...
    fun `business exception maps to error code status and message`() {
        val request = mockk<HttpServletRequest> { every { requestURI } returns "/api/users/me" }

        val response = handler.handleBusinessException(UserException(ErrorCode.USER_NOT_FOUND), request)

        assertEquals(HttpStatus.NOT_FOUND, response.statusCode)
        assertEquals(ErrorCode.USER_NOT_FOUND.message, response.body?.detail)
        assertEquals(ErrorCode.USER_NOT_FOUND.name, response.body?.title)
    }
    ...
    fun `business exception keeps explicit message`() {
        ...
        val response = handler.handleBusinessException(
            UserException(ErrorCode.USER_NOT_FOUND, "찾을 수 없는 회원입니다."),
            request,
        )
